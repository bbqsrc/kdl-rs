//! Types for schema validation constraints.
...
impl Validations {
    ...
    pub(super) fn from_node(node: &KdlNode) -> Self {
        ...
        for node in children.nodes() {
            if node.name().value() == "pattern" {
                if let Some(pattern) = node.get(0).and_then(|v| v.as_string()) {
        ...
        // format constraint
        for node in children.nodes() {
            if node.name().value() == "format" {
                if let Some(fmt) = node.get(0).and_then(|v| v.as_string()) {
                    validations.formats.push(ValueFormat::from_str(fmt));
                }
            }
        }
        ...
        for node in children.nodes() {
            if node.name().value() == "%" {
                if let Some(v) = node.get(0).and_then(|v| v.as_float()) {
                    validations.multiple_of.push(v);
                } else if let Some(v) = node.get(0).and_then(|v| v.as_integer()) {
        ...
        // > constraint
        if let Some(gt_node) = children.get(">") {
        ...
        // >= constraint
        if let Some(gte_node) = children.get(">=") {
        ...
        // < constraint
        if let Some(lt_node) = children.get("<") {
        ...
        // <= constraint
        if let Some(lte_node) = children.get("<=") {
        ...
        // tag constraint (for value type annotations)
    ...
    pub(super) fn check(
        ...
        if let KdlValue::String(s) = value {
            ...
            // Format checks (simplified - full validation would require more complex logic)
            for format in &self.formats {
                if !self.check_string_format(s, format) {
                    errors.push(KdlDiagnostic {
                        input: input.clone(),
                        span,
                        message: Some(format!("Value does not match format: {:?}", format)),
        ...
        if let Some(num) = num_value {
            ...
            if let KdlValue::Integer(i) = value {
                for format in &self.formats {
                    if !self.check_integer_format(*i, format) {
                        errors.push(KdlDiagnostic {
                            input: input.clone(),
                            span,
                            message: Some(format!("Integer does not fit format: {:?}", format)),
                            label: Some("format mismatch".into()),
                            help: None,
                            severity: Severity::Error,
                        });
            ...
            if let KdlValue::Float(f) = value {
                for format in &self.formats {
                    if !self.check_float_format(*f, format) {
                        errors.push(KdlDiagnostic {
                            input: input.clone(),
                            span,
                            message: Some(format!("Float does not fit format: {:?}", format)),
                            label: Some("format mismatch".into()),
                            help: None,
                            severity: Severity::Error,
                        });
    ...
    fn check_string_format(&self, s: &str, format: &ValueFormat) -> bool {
        match format {
            ValueFormat::DateTime => {
                // ISO8601: YYYY-MM-DDTHH:MM:SS[.sss][Z|±HH:MM]
                // Try parsing as a timestamp or datetime
            ...
            // Formats that need additional libraries or are too complex for basic validation
            ...
            // Numeric formats don't apply to strings
            _ => true,
        }
    }
    ...
    fn check_float_format(&self, f: f64, format: &ValueFormat) -> bool {
        match format {
            ...
            ValueFormat::F64 => true, // Native f64, always valid
            ValueFormat::Decimal64 | ValueFormat::Decimal128 => true, // Would need decimal lib
            _ => true,
        }
    }
}
